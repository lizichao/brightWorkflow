...
<%}%>
<bc:vpage file="workflow/workspace/processmonitorback.view.xml">
<bc:vfuncs />
<bc:vrecords />
<bc:vstores />
<bc:vpanels />
...
<%if(!"2".equals(tp)){%>
	Ext.onReady(function() {
		...
		var _mainPanel=new jsp.workflow.workspace.processmonitorback();
		new Ext.Viewport({layout:"fit",items:_mainPanel.MainPanel});
		_mainPanel.__jrafonload(<%=opts%>);
		JrafUTIL.putCmp("jsp.workflow.workspace.processmonitorback",_mainPanel);
	});
<%}else{%>
	...
	<%if("true".equals(request.getAttribute("__gjfile__"))){%>JrafUTIL.scriptLoader("<%=request.getContextPath()%>/workflow/workspace/processmonitorback.js",true,__mfunc);<%}else{%>__mfunc();<%}%>
<%}%>
</bc:vpage>
<%if(!"2".equals(tp)){%>
</head>
