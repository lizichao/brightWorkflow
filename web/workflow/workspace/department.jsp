...
<%}%>
<bc:vpage file="workflow/workspace/department.view.xml">
<bc:vfuncs />
<bc:vrecords />
<bc:vstores />
<bc:vpanels />
...
<%if(!"2".equals(tp)){%>
	Ext.onReady(function() {
		...
		var _mainPanel=new jsp.workflow.workspace.department();
		new Ext.Viewport({layout:"fit",items:_mainPanel.MainPanel});
		_mainPanel.__jrafonload(<%=opts%>);
		JrafUTIL.putCmp("jsp.workflow.workspace.department",_mainPanel);
	});
<%}else{%>
	var __mfunc1=function(){
		var _mainPanel=new jsp.workflow.workspace.department();
	...
	<%if("true".equals(request.getAttribute("__gjfile__"))){%>JrafUTIL.scriptLoader("<%=request.getContextPath()%>/workflow/workspace/department.js",true,__mfunc1);<%}else{%>__mfunc1();<%}%>
<%}%>
</bc:vpage>
<%if(!"2".equals(tp)){%>
</head>
