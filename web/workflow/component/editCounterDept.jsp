...
var deptData = paramObject.infoData;
...
$(document).ready(function(){
	 $("#departmentDiv").css("height","452px")
	   ...
	   for (var i=0;i<selectdDept.length;i++){
		   var selectdDeptEach = selectdDept[i];
		   var isMajor = selectdDeptEach.isMajor;
		   if(isMajor == '1'){
			   majorDept.push(selectdDeptEach);
		   }else{
			   assistDept.push(selectdDeptEach);
		   }
	   }
	   
	   if(majorDept.length==0){
		   $('input[name="wayRadio"][value=2]').attr('checked','checked');
	   }
	 
	   
	   var newSelectdDept = majorDept.concat(assistDept);
	   
	   for (var i=0;i<newSelectdDept.length;i++){
		      var configHandlerEach = newSelectdDept[i];
...
<div class="hi-hd">
<h3><i class="ico-xzclr"></i>选择部门</h3> 
</div>
