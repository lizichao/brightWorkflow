...
(function($, window, document, undefined) {
   ...
   $.fn.selectpick = function(options) {
      ...
      return this.each(function(elem_id) {
         ...
         var margin = $(obj).attr("margin");
         var addwidth = $(obj).attr("addwidth");
         var lesstop = $(obj).attr("lesstop");
         // 生成的div的样式
         if (margin==undefined) {
         	margin = 0;
         }
         if (addwidth==undefined) {
        	addwidth = 0;
         }
         if (lesstop==undefined) {
        	 lesstop = 0;
          }
		...
		top -= parseInt(lesstop); 
         $(".selectpick_div_" + elem_id).css({
            "height": settings.height,
            "width": settings.width + parseInt(addwidth),
            "left": left,
            "top": top 
         });
         ...
         $(".selectpick_div_" + elem_id + ",#selectpick_span_" + elem_id + ",#selectpick_options_" + elem_id + "").bind("click", function(event) {
            ...
            if ($(".selectpick_ul_" + elem_id + " li").length > 0) {
               ...
               $(".selectpick_options_" + elem_id).css({
                  "width": settings.width + 5 + parseInt(addwidth),
                  "left": left,
               ...
               $(".selectpick_ul_" + elem_id + " li").bind("click", function() {
                  ...
                  if (settings.onSelect != undefined && settings.onSelect != "" && typeof settings.onSelect == "function") {
                     settings.onSelect(elem_id,$(this).children("label").first().text(), $(this).children("label").first().next().text());
                  }
               });
            }
