Ext.namespace("jsp.platform.sm");jsp.platform.sm.processmonitor=function(){var __caches__=[];this.__caches__=__caches__;var _editInfoWin; function openEditInfoWin(_rec){ _editInfoWin=Ext.getCmp('editInfoWin'); if(!_editInfoWin){_editInfoWin=new Ext.Window({title:'数据录入',id:'editInfoWin',layout:'fit',width:600,height:300,closeAction:'hide',plain:true,modal: true,items:EditInfoForm});} _editInfoWin.show();EditInfoForm.getForm().reset();EditInfoForm.getForm().getForm().loadRecord(_rec);EditInfoForm.record=_rec;}var ProcessMonitorRec=Ext.data.Record.create([  {
    xtype : "Field",
    name : "processName",
    width : 150,
    fieldLabel : "流程名称",
...
this.ProcessMonitorRec=ProcessMonitorRec;this.__caches__.push(ProcessMonitorRec);var ProcessMonitorStore=Ext.create({
  xtype : "Store",
  ...
  api : {
    read :     {
      sysName : "pcmc",
      oprID : "ProcessMonitor",
      actions : "queryProcessInstance"
  },
    create :     {
      sysName : "pcmc",
      oprID : "ProcessMonitor",
...
this.ProcessMonitorStore=ProcessMonitorStore;this.__caches__.push(ProcessMonitorStore);var EditInfoForm=Ext.create({
  xtype : "panel",
...
this.EditInfoForm=EditInfoForm;this.__caches__.push(EditInfoForm);var MainPanel=new Ext.Panel({layout:'fit',border:false,bodyBorder:false,items:[  {
    ...
    items : [      {
        ...
        colModel : new Ext.grid.ColumnModel([new Ext.grid.CheckboxSelectionModel(),{width:100,sortable:true,header:'流程名称',dataIndex:'processName',renderer:Ext.util.Format.paramRenderer('undefined','')}]),
