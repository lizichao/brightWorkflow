...
<jsp:include page="/masterreview/personalleader/personalRenderTemplate.jsp" flush="true" >
 <jsp:param name="approveType" value="personalApprove" />
...
$(document).ready(function(){	
	...
	//Headmaster.findMasterBaseInfo(<%=masterReviewVO%>.headerMasterId);
	Headmaster.bulidMsaterView(<%=masterReviewVO%>,Headmaster.personalApprove);
	
	$(".qa").hover(
			function(){
				 b=setTimeout(function(){$.fancybox.open({href:"/masterreview/public/gradeStandard.jsp",type:'iframe',width:777,height:488,padding:0,margin:0,closeBtn:false,iframe:{scrolling:'no',preload:false}});},200) ;
...
function beforeSubmit(formJsonData){
	...
	formJsonData.baseinfo_grade=  $("#baseinfo_grade").val() ||'0';
	formJsonData.work_experience_grade=  $("#work_experience_grade").val()||'0';
	formJsonData.education_grade=  $("#education_grade").val()||'0';
	formJsonData.professional_title_grade=  $("#professional_title_grade").val()||'0';
	formJsonData.management_difficulty_grade=  $("#management_difficulty_grade").val()||'0';
	formJsonData.management_difficulty_grade_ago=  $("#management_difficulty_grade_ago").val()||'0';
	formJsonData.paper_grade= Headmaster.getOptionGrades('paper_grade_index','paper_grade') ;
	formJsonData.work_publish_grade=  $("#work_publish_grade").val()||'0';
	formJsonData.subject_grade=  $("#subject_grade").val()||'0';
	formJsonData.personal_award_grade=  $("#personal_award_grade").val()||'0';
	formJsonData.school_award_grade=  $("#school_award_grade").val()||'0';
	
	formJsonData.studyTrain_grade=  $("#studyTrain_grade").val()||'0';
	formJsonData.schoolReform_grade=  $("#schoolReform_grade").val()||'0';
	formJsonData.socialDuty_grade=  $("#socialDuty_grade").val()||'0';
	formJsonData.accident_grade=  $("#accident_grade").val()||'0';
	formJsonData.punishment_grade=  $("#punishment_grade").val()||'0';
	
	formJsonData.sumGrade=  $("#sumGrade").val()||'0';
}

function afterSubmit(){
	 window.location.href = "/masterreview/personalleader/personalleaderIndex.jsp";
}

function submitPersonalTask(){
	var submitFlow = "";
	if(Brightcom.workflow.getTaskDefKey() == 'usertask6'){
		submitFlow = 'flow18';
	}else{
		submitFlow = 'flow13';
	}
	Brightcom.workflow.completeButtonTask(submitFlow,Brightcom.workflow.taskType);
}
...
function countSumGrade(){
	...
	var paper_grade =Headmaster.getOptionGrades('paper_grade_index','paper_grade');
...
function gradeStandard(obj){
	$(obj).mouseover(
		function(){
			$.fancybox.open({href:"/masterreview/public/gradeStandard.jsp",type:'iframe',width:777,height:488,padding:0,margin:0,closeBtn:false,iframe:{scrolling:'no',preload:false}});
	    }
	).mouseout(function(){
		 $.fancybox.close();
	   }
	)
	//window.open("/masterreview/public/gradeStandard.jsp");     
	//$.fancybox.open({href:"/masterreview/public/gradeStandard.jsp",type:'iframe',width:777,height:488,padding:0,margin:0,closeBtn:false,iframe:{scrolling:'no',preload:false}});
}
</script>
<input type="hidden" id="workflowSubmitAction" name="workflowSubmitAction" value="headmaster@@masterReviewAction@@completePersonnelLeaderTask">
<input type="hidden" id="id" name="id" value="">
<input type="hidden" id="headerMasterId" name="headerMasterId" value="">

<input type="hidden" id="paper_grade_index" name="paper_grade_index" value="0">

	
<div class="container">
		<div class="xiao_bg">
			<div class="tit1">申报人信息</div>
			<div class="tabletit">量化指标评价</div>
 ...
 -->
			<div class="tabletit"><a href="javascript:void();" onclick="submitPersonalTask()" style="background-color:#44b549">提交</a></div>
		</div>
	</div>
