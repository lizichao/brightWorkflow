...
<div class="load">
	<div class="loadin">
		...
		<span class="close" onclick="windowclose();"><img style="cursor:pointer;" src="/images/close.png" alt="" /></span>
			<span class="loadspan">退回</span>
		</div>
		<div class="loadbd">
			 <textarea id="work_report" rows="7" cols="32"></textarea>
		
			<button class="btn" onclick="rollbackProcess()" style="cursor:pointer;background:#44b549">确定</button>
		</div>
	</div>
</div>
</body>
...
function updatePwd(){
    ...
    bcReq.setSuccFn(function(data,status){
         top.toastr.success("修改成功，请牢记新密码！");
         top.$.fancybox.close();
    });
    bcReq.setFailFn(function(data,status){
		top.toastr.error("修改失败!"+data.error_description);
	});
    bcReq.postData();
}
</script>
