...
<%}%>
<bc:vpage file="masterreview/headmaster/headmaster.view.xml">
<bc:vfuncs />
<bc:vrecords />
<bc:vstores />
<bc:vpanels />
...
<%if(!"2".equals(tp)){%>
	Ext.onReady(function() {
		...
		var _mainPanel=new jsp.masterreview.headmaster.headmaster();
		new Ext.Viewport({layout:"fit",items:_mainPanel.MainPanel});
		_mainPanel.__jrafonload(<%=opts%>);
		JrafUTIL.putCmp("jsp.masterreview.headmaster.headmaster",_mainPanel);
	});
<%}else{%>
	...
	<%if("true".equals(request.getAttribute("__gjfile__"))){%>JrafUTIL.scriptLoader("<%=request.getContextPath()%>/masterreview/headmaster/headmaster.js",true,__mfunc);<%}else{%>__mfunc();<%}%>
<%}%>
</bc:vpage>
<%if(!"2".equals(tp)){%>
</head>
