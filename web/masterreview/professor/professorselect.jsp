...
<%}%>
<bc:vpage file="masterreview/professor/professorselect.view.xml">
<bc:vfuncs />
<bc:vrecords />
<bc:vstores />
<bc:vpanels />
...
<%if(!"2".equals(tp)){%>
	Ext.onReady(function() {
		...
		var _mainPanel=new jsp.masterreview.professor.professorselect();
		new Ext.Viewport({layout:"fit",items:_mainPanel.MainPanel});
		_mainPanel.__jrafonload(<%=opts%>);
		JrafUTIL.putCmp("jsp.masterreview.professor.professorselect",_mainPanel);
	});
<%}else{%>
	...
	<%if("true".equals(request.getAttribute("__gjfile__"))){%>JrafUTIL.scriptLoader("<%=request.getContextPath()%>/masterreview/professor/professorselect.js",true,__mfunc);<%}else{%>__mfunc();<%}%>
<%}%>
</bc:vpage>
<%if(!"2".equals(tp)){%>
</head>
