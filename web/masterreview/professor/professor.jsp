...
<%}%>
<bc:vpage file="masterreview/professor/professor.view.xml">
<bc:vfuncs />
<bc:vrecords />
<bc:vstores />
<bc:vpanels />
...
<%if(!"2".equals(tp)){%>
	Ext.onReady(function() {
		...
		var _mainPanel=new jsp.masterreview.professor.professor();
		new Ext.Viewport({layout:"fit",items:_mainPanel.MainPanel});
		_mainPanel.__jrafonload(<%=opts%>);
		JrafUTIL.putCmp("jsp.masterreview.professor.professor",_mainPanel);
	});
<%}else{%>
	...
	<%if("true".equals(request.getAttribute("__gjfile__"))){%>JrafUTIL.scriptLoader("<%=request.getContextPath()%>/masterreview/professor/professor.js",true,__mfunc);<%}else{%>__mfunc();<%}%>
<%}%>
</bc:vpage>
<%if(!"2".equals(tp)){%>
</head>
