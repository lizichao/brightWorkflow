...
function bulidNoApproveData(){
	...
	var bcReq = new BcRequest('headmaster','professorAction','findNoApproveData');
	bcReq.setExtraPs({
...
function bulidApproveData(){
	...
	var bcReq = new BcRequest('headmaster','professorAction','findApproveData');
	bcReq.setExtraPs({'username':approveText});
	bcReq.setSuccFn(function(data,status){
		 $("#approveDiv").empty();
		 if(data.Data.length>0){
