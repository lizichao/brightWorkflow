...
@Component
public class ReceiveUpdateListener {
    public void notify(DelegateExecution execution) throws Exception {
        String businessKey = execution.getProcessBusinessKey();
        String currentUserid = (String) ApplicationContext.getRequest().getSession().getAttribute("userid");

        String officeApprover = ApplicationContextHelper.getJdbcTemplate().queryForObject("select officeApprover from workflow_receivefile where id=?", String.class, businessKey);
